...
class Mastodon_api {
    ...
    private function _post ($url, $parameters = array()) {
        ...
        $response = $this->get_content_remote_get($url,$params);
        return $response;
    }
    ...
    private function _patch ($url,$parameters = array()) {
        ...
        $url = $this->mastodon_url.$url;
        $response = $this->get_content_remote_get($url,$parameters);
        return $response;
    }
     ...
     * @return  array       $data
     */
    public function get_content_remote_get ($url,$parameters = array()) {
        $data = array();

        // set USERAGENT
	    if (isset($_SERVER['HTTP_USER_AGENT'])) {
		    $parameters['headers'][] = $_SERVER['HTTP_USER_AGENT'];
	    } else {
		    // default IE11
		    $parameters['headers'][] = 'Mozilla/5.0 (Windows NT 10.0; WOW64; Trident/7.0; rv:11.0) like Gecko';
	    }
	    ...
	    if( is_wp_error($response)){
		    ...
		    $data['html'] = json_decode($response['body'], true);
